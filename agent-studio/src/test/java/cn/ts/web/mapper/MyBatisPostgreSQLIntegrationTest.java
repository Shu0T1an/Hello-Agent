...
import cn.ts.web.infra.mcp.mapper.McpConnectionConfigMapper;
...
class MyBatisPostgreSQLIntegrationTest {
    ...
    void requireDatabase() {
        try (Connection ignored = DriverManager.getConnection(TEST_DB_URL, TEST_DB_USER, TEST_DB_PASSWORD)) {
            // no-op
        } catch (Exception e) {
            Assumptions.assumeTrue(false, "PostgreSQL unavailable, skip integration tests: " + e.getMessage());
        }
    }
